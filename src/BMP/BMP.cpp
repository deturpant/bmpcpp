...
BMP::BMP(int32_t width, int32_t height, bool has_alpha) {
    ...
    if (width <= 0 || height <= 0) throw KVA::MyException{"The image width and height must be positive numbers."};
...
void BMP::load(const std::string & file_path) {
    ...
    if (!input) throw KVA::MyException{"Unable to open the input image file!"};

    // считываем заголовочную структуру изображения
    input.read((char*)&m_file_header, sizeof(m_file_header));

    // проверяем формат изображения
    if (m_file_header.file_type != 0x4D42) throw KVA::MyException{"Unrecognized file format!"};
    ...
    if (m_info_header.bit_count == 32) {
        ...
        // если не содержит - выбрасываем исключение
            ...
            throw KVA::MyException{"Error! Unrecognized file format!"};
        }
    }
    ...
    if (m_info_header.height < 0)
        throw KVA::MyException{"The program can treat only BMP images with the origin in the bottom left corner!"};
...
void BMP::save(const std::string & file_path) {
    ...
    // проверяем, открылся ли файл
    if (!output) throw KVA::MyException{"Unable to open the input image file!"};
    ...
    // выбрасываем исключение (данная программа обрабатывает только 24- или 32-битные изображения)
        ...
        throw KVA::MyException{"The program can treat only 24 or 32 bits per pixel BMP files"};
    }
}
...
void BMP::fillRegion(uint32_t x0, uint32_t y0, uint32_t width, uint32_t height,
    ...
    if (x0 + width > (uint32_t)m_info_header.width || y0 + height > (uint32_t)m_info_header.height)
        throw KVA::MyException{"The region does not fit in the image!"};
...
void BMP::_checkColorHeader(ColorHeader &color_header) {
    ...
    if (expected_color_header.red_mask   != color_header.red_mask   ||
        ...
        throw KVA::MyException{"Unexpected color mask format! "
    ...
    if (expected_color_header.color_space_type != color_header.color_space_type) {
        throw KVA::MyException{"Unexpected color space type! The program expects sRGB values"};
    }
}
...
void BMP::createCircle() {
    ...
    int32_t x0 = width/2;
    int32_t y0 = radius;
    //std::cout << "Center x = " << x0 << " | y = " << y0 << "\nRadius = " <<radius << "\nCount of channel: " << channels;
    // проходимся по всем пикселям изображения
    for (uint32_t y = 0; y < height; ++y) {
        for (uint32_t x = 0; x < width; ++x) {
            if (((x-x0)*(x-x0) + (y-y0)*(y-y0))>radius*radius) {
                // цвет каждого пикселя уводим в белый
                m_data.at(channels * (y * m_info_header.width + x) + 0) = 255;
                m_data.at(channels * (y * m_info_header.width + x) + 1) = 255;
                m_data.at(channels * (y * m_info_header.width + x) + 2) = 255;

                // если имеем 4 канала (32-битное изображение)
                if (channels == 4) {
                    // задаем компонент прозрачности
                    m_data.at(channels * (y * m_info_header.width + x) + 3) = 0;
                }
            }
        }
    }
